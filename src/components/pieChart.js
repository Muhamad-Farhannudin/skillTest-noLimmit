...
const PieChart = () => {
  ...
  const [dateRange, setDateRange] = useState({
    //Menggunakan tanggal default, agar tampilan pieChart muncul dari tahun 2013 sesuai API
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // karena kita akan mengambil data untuk lineChart jadi buat variable bebas contohnya pies
        const pies = data.data;

        // Ubah data API agar berurutan dari 2013 sampai 2020
        setLine(pies.reverse());
      } catch (error) {
  ...
  const data = {
    ...
    datasets: [
      {
        label: "Population",
        backgroundColor: ["rgb(243, 61, 60)", "rgb(254, 162, 59)", "rgb(252, 246, 78)", "rgb(127, 242, 61)", "rgb(68, 220, 233)", "rgb(67, 87, 245)", "rgb(240, 61, 213)", "rgb(1, 133, 119)"],
        data: populates
      },
    ],
  };
  ...
  return (
    <div className="w-1/2">
      <h3 className="text-blueDark text-4xl font-semibold mb-5">Pie Chart</h3>
      <DateRangePicker
